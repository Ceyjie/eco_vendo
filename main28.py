...
# ═══════════════════════════════════════════════════════════
# Small bottle   : 9–16g   → bottom IR only → 1 pt
# Nature Spring  : 16–19g  → both IR        → 2 pts
# Big bottle     : 29–35g  → both IR        → 2 pts
WEIGHT_MIN_VALID  =  8   # below = not a bottle
WEIGHT_SMALL_MIN  =  9
WEIGHT_SMALL_MAX  = 16
...
# ═══════════════════════════════════════════════════════════
def classify_bottle(weight_g, both_ir, bottom_only):
    """Returns (points, label, lcd_line1, lcd_line2)"""
    if weight_g < WEIGHT_MIN_VALID:
        return 0, None, "   NOT A BOTTLE!   ", "  Bottles only plz "
    ...
    if both_ir:
        if WEIGHT_NATURE_MIN <= weight_g <= WEIGHT_NATURE_MAX:
            ...
            return 0, None, "  INVALID BOTTLE   ", f"  Weight:{weight_g:.0f}g OOR  "
    ...
    return 0, None, "   BOTTLES ONLY!   ", "  Try again...     "

# ═══════════════════════════════════════════════════════════
# BOTTLE PROCESSING
# ═══════════════════════════════════════════════════════════
def process_bottle(both_ir, bottom_only):
    """Triggered in background thread when IR detects bottle"""
    with bottle_lock:
        # Countdown — give user 3s to hold bottle steady
        for i in range(3, 0, -1):
            lcd_write([
                "    VERIFYING...    ",
                f"   Please wait {i}s...  "[:20],
                "   Hold bottle on   ",
                "     sensor...      "
            ])
            time.sleep(1)
        ...
        pts, label, line1, line2 = classify_bottle(weight, both_ir, bottom_only)
    ...
    while True:
        ...
        if session_data["state"] == "INSERTING" and now >= ir_cooldown \
            ...
            if bot_triggered:
                both_ir    = (top == 0)
                bottom_only = (top == 1)
                ir_cooldown = now + 6.0  # block IR during processing
                threading.Thread(
                    target=process_bottle,
                    args=(both_ir, bottom_only),
                    daemon=True
                ).start()
