...
def init_scale():
    ...
    # ── Auto tare: wait for stable reading ───────────────────────────────────
    print("Auto-zeroing")
    attempt = 0
    while True:
        samples = [read_raw() for _ in range(10)]
        ...
        if len(samples) < 5:
            ...
            time.sleep(0.2)
            attempt += 1
            if attempt >= MAX_RETRIES * 5:
                print("\nWARN: Could not stabilize, using current reading.")
                break
            continue
        ...
        sys.stdout.write(f"\r  Raw: {avg:>10.0f}  Variance: {variance:>6.0f}  {'STABLE' if variance < 200 else 'waiting...'} ")
        sys.stdout.flush()

        if variance < 200:
            tare_offset = avg
            empty_count = 0
            print(f"\nZeroed. Offset={tare_offset:.0f}  Ready to weigh!")
            return True
