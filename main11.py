...
# --- LCD ENGINE (Optimized for Responsiveness) ---
lcd = None
current_lcd_lines = ["", "", "", ""]
...
def lcd_write(new_lines):
    ...
    try:
        ...
        for i, line in enumerate(new_lines):
            # Only update I2C if text changed to keep CPU/Bus free
...
# --- HARDWARE LOOP (Responsive Button & Sensor Logic) ---
def hardware_loop():
    ...
    while True:
        ...
        # 2. Point Logic (Active High: 0 = Object Detected)
        if session_data["state"] == "INSERTING":
            bottom_tripped = (gpio_read(PIN_IR_BOTTOM) == 0)
            top_tripped = (gpio_read(PIN_IR_TOP) == 0)

            if bottom_tripped:
                # NEW POINT RULES
                if top_tripped:
                    session_data["count"] += 2 # Tall bottle (Both high)
                else:
                    session_data["count"] += 1 # Small bottle (Only bottom)
                
                session_data["last_activity"] = now
                beep(1)
                time.sleep(0.8) # Brief lockout so bottle can pass
...
def display_manager():
    while True:
        ...
        if s == "IDLE":
            ...
            lcd_write([
                "   INSERT BOTTLE", 
                f"    POINTS: {int(session_data['count'])}", 
                f"    TIME: {int(session_data['count']*5)}m", 
                "B3: CONFIRM"
            ])
        time.sleep(0.1)
...
def get_status():
    ...
    return jsonify({
        ...
        "session": int(session_data["count"]),
        "points": user_pts,
