...
def servo_send_pulse(pulse_ms):
    """Tight busy-wait PWM — no OS sleep, perf_counter only for accuracy."""
    ...
    servo_set_pin(0)
    # Fill remainder of 20ms period
    while (time.perf_counter() - t0) < period_s: pass

def servo_pwm_thread():
    """Send pulses only when moving, then stop to prevent jitter at standby."""
    pulse_ms    = PULSE_MIN_MS + (90 / 180.0) * (PULSE_MAX_MS - PULSE_MIN_MS)
    pulse_count = 0
    MAX_PULSES  = 50   # ~1 second of pulses (50Hz x 1s) then stop

    while servo_active:
        try:
            pulse_ms    = _servo_queue.get_nowait()
            pulse_count = 0   # reset counter on new command
        except queue.Empty:
            pass
        ...
        if pulse_count < MAX_PULSES:
            ...
            time.sleep(0.02)  # idle — no pulses, no jitter
...
def hardware_loop():
    ...
    while True:
        ...
        # Both IR active LOW — either 1→0 triggers bottle detection
