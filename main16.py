...
def hardware_loop():
    ...
    ir_cooldown = 0  # IR cooldown timestamp (no sleep, safe when sensor not connected)
    ...
    while True:
        ...
        for p in btn_pins:
            ...
            if val == 0 and last_val[p] == 1:
                if (now - last_press[p]) > 0.3:  # 300ms per button independently
                    last_press[p] = now
