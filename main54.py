...
# ═══════════════════════════════════════════════════════════
#  Small bottle  :  9 – 15g  → 1 pt
#  Nature Spring : 16 – 19g  → 2 pts
#  Big bottle    : 20 – 39g  → 2 pts   (covers 20-28g gap too)
WEIGHT_MIN_VALID  =  8
WEIGHT_SMALL_MIN  =  9
WEIGHT_SMALL_MAX  = 15   # fixed: no overlap with Nature Spring
WEIGHT_NATURE_MIN = 16
WEIGHT_NATURE_MAX = 19
WEIGHT_BIG_MIN    = 20   # fixed: was 29, now catches 20-39g
WEIGHT_BIG_MAX    = 39

# ═══════════════════════════════════════════════════════════
# SERVO — PA10, /dev/mem direct bit-bang
# ═══════════════════════════════════════════════════════════
PIN_SERVO      = "10"
PULSE_MIN_MS   = 0.5
PULSE_MAX_MS   = 2.5
...
# ═══════════════════════════════════════════════════════════
# LCD  —  two-tier write system
#   lcd_write()       → used by display_manager; skips if process_bottle holds lock
#   lcd_write_force() → used by process_bottle; writes directly (already holds lock)
# ═══════════════════════════════════════════════════════════
lcd = None
current_lcd_lines = ["", "", "", ""]
...
def _lcd_write_raw(new_lines):
    """Write to LCD — no lock acquired here."""
...
def lcd_write(new_lines):
    """display_manager path — skips if process_bottle holds lcd_lock."""
    if lcd_lock.locked():
        return
    with lcd_lock:
...
def lcd_write_force(new_lines):
    """process_bottle path — called while already holding lcd_lock."""
    _lcd_write_raw(new_lines)
...
def _process_bottle_inner():
    """Called with lcd_lock already held — use lcd_write_force() only."""
...
# ═══════════════════════════════════════════════════════════
# DISPLAY MANAGER  — 20fps, skips when process_bottle holds lcd_lock
# ═══════════════════════════════════════════════════════════
def display_manager():
    last_lines = []
