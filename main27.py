...
# Small bottle  : 12–16g  → 1 point  (bottom IR only)
...
def classify_bottle(weight_g, both_ir, bottom_ir_only):
    ...
    Both IR + 16-19g  = Nature Spring  → 2 pts
    Both IR + 29-35g  = Big bottle     → 2 pts
    Bottom only + 12-16g = Small       → 1 pt
    ...
    if both_ir:
        if WEIGHT_NATURE_MIN <= weight_g <= WEIGHT_NATURE_MAX:
            return 2, "NATURE", "NATURE SPRING +2pt"
...
def safe_lcd(text):
    """Ensure string is exactly 20 chars, ASCII safe, no special chars."""
    cleaned = ""
    for ch in str(text):
...
def process_bottle(both_ir, bottom_ir_only):
    ...
    for i in range(3, 0, -1):
        lcd_write([
            safe_lcd("   VERIFYING..."),
            safe_lcd(f"  Please wait {i}s"),
            safe_lcd("  Hold bottle..."),
            safe_lcd("")
        ])
        time.sleep(1)
    ...
    if pts > 0:
        ...
        lcd_write([
            safe_lcd("  REMOVE BOTTLE"),
            safe_lcd("  FROM SENSOR..."),
            safe_lcd(""),
            safe_lcd("")
        ])
        ...
        lcd_write([
            safe_lcd(f"  +{pts} PT{'S' if pts > 1 else ''}! WELL DONE!"),
            safe_lcd(f"  Total: {total} pts"),
            safe_lcd("  Thank you for"),
            safe_lcd("  recycling!")
        ])
        time.sleep(2)
        ...
        lcd_write([
            safe_lcd("  INVALID ITEM!"),
            safe_lcd(msg),
            safe_lcd("  BOTTLES ONLY!"),
            safe_lcd("  Try again...")
        ])
        beep_now(3)
        time.sleep(2)
