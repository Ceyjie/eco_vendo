...
session_data = {
    ...
    "count": 0, 
    "selected_slot": 0,
    "add_time_choice": 0, # 0 for No, 1 for Yes
    "last_activity": time.time()
}
slot_status = {0: 0, 1: 0, 2: 0, 3: 0}
relay_threads = {} # Track active timers
...
def run_relay_timer(slot, seconds):
    ...
    while slot_status[slot] > 0: # Check the dictionary value directly
        time.sleep(1)
        slot_status[slot] -= 1
    gpio_write(PINS_RELAYS[slot], 0)
    slot_status[slot] = 0

def start_or_add_timer(slot, additional_seconds):
    if slot_status[slot] > 0:
        slot_status[slot] += additional_seconds
    else:
        slot_status[slot] = additional_seconds
        threading.Thread(target=run_relay_timer, args=(slot, additional_seconds), daemon=True).start()
...
def handle_press(pin):
    ...
    if pin == PIN_BTN_START and state == "IDLE":
        ...
        elif state == "ADD_TIME_PROMPT":
            beep(1)
            session_data["add_time_choice"] = 1 - session_data["add_time_choice"] # Toggle 0 and 1
        ...
        if state == "INSERTING":
            ...
            if slot_status[slot] > 0:
                ...
                session_data["add_time_choice"] = 1 # Default to Yes
            else:
...
def finalize_transaction():
    ...
    beep(2)
    start_or_add_timer(slot, total_sec)
...
def hardware_loop():
    ...
    while True:
        ...
        if state := session_data["state"] == "INSERTING":
        ...
        if state not in ["IDLE", "THANK_YOU"]:
...
def display_manager():
    while True:
        ...
        if state == "IDLE":
            ...
            choice_text = "> YES   NO " if session_data["add_time_choice"] == 1 else "  YES > NO "
            lcd_write(["   ADD MINUTES TO", f"   {SLOT_NAMES[session_data['selected_slot']]}?", choice_text, "B2:MOVE B3:OK"])
