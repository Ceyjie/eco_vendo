...
def on_btn_confirm():
    if ui_state["state"] == "INSERTING":
        ...
        slot = ui_state["selected_slot"]
        # Logic: 1 bottle = 5 minutes (300 seconds)
...
def redeem_web(slot, pts):
    # Calculate time: 1 point = 300 seconds (5 mins)
    ...
    # 1. Start the hardware timer immediately
    threading.Thread(target=run_relay_timer, args=(slot, total_seconds), daemon=True).start()
    
    # 2. Update the LCD to show the web-triggered activation
    ui_state["state"] = "DONE"
    lcd_write([
        "   WEB ACTIVATED!",
        f" Slot: {SLOT_NAMES[slot]}",
    ...
    # 3. Brief delay so the user sees the activation on the LCD, then return to IDLE
    def reset_ui():
        time.sleep(3)
        ui_state["state"] = "IDLE"
