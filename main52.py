...
def _process_bottle_inner():
    ...
    if pts > 0:
        ...
        lcd_write_force([
            "   INSERT BOTTLE    ",
            f"   Bottles: {cnt}        "[:20],
            f"   Time: {cnt*5:.0f}m         "[:20],   # ← 5 mins per point
            "   B3: Confirm      "
        ])
...
def finalize_transaction():
    ...
    beep(2)
    start_or_extend_relay(slot, pts * 300)   # ← 5 mins (300s) per point
...
def display_manager():
    while True:
        ...
        if lcd_lock.locked():
            time.sleep(0.05)   # ← faster check when locked
            continue
        ...
        if s == "IDLE":
            ...
            lcd_write(["      SELECT        ",
                       ...
                       f"    FOR {session_data['count']*5} MINS   "[:20],   # ← 5 mins per point
                       "   B3: CONFIRM      "])
        elif s == "ADD_TIME_PROMPT":
