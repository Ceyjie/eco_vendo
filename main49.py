...
_ir_reset = False  # flag to reset IR state on session start
...
def handle_physical_press(pin):
    ...
    if pin == PIN_BTN_START and s == "IDLE":
        ...
        # Force IR last state to HIGH (idle) so no false trigger on start
        global _ir_reset
        _ir_reset = True
...
def classify_bottle(weight_g):
    if weight_g < WEIGHT_MIN_VALID:
        ...
        return 0, None, "  INVALID BOTTLE!   ", f"  {weight_g:.0f}g not valid     "[:20]

def process_bottle():
    with bottle_lock:
        # Weigh over 5 seconds — show live weight
        lcd_write(["  BOTTLE DETECTED   ",
                   "  Weighing...       ",
                   "  Hold still...     ",
                   "                    "])
        ...
        while time.time() - start_time < 5.0:
            ...
            if w is not None and w > WEIGHT_MIN_VALID:
                ...
                lcd_write(["  BOTTLE DETECTED   ",
                           f"  {avg:6.1f} g          "[:20],
                           f"  Reading {len(samples)}/5       "[:20],
                           "  Hold still...     "])
            if len(samples) >= 5:
                final = sorted(samples[-5:])
                if (final[-1] - final[0]) <= 5.0:
                    break
            time.sleep(0.4)
        ...
        if pts > 0:
            lcd_write(["    ACCEPTED!       ", line1, line2, "   Please wait...   "])
...
def hardware_loop():
    ...
    ir_cooldown = 0
    ir_last_bot = 1   # 1 = idle (beam not broken)
    ir_last_top = 1   # 1 = idle (beam not broken)
    ...
    while True:
        ...
        if session_data["state"] == "INSERTING" and now >= ir_cooldown and not admin_data["active"]:
            global _ir_reset
            # Reset IR last state to HIGH when session just started
            if _ir_reset:
                ir_last_bot = 1
                ir_last_top = 1
                _ir_reset = False
            ...
            # Active HIGH at rest, goes LOW when bottle passes
...
def display_manager():
    ...
    while True:
        ...
        # Only write to LCD if content changed — prevents slow redundant I2C writes
        if lines != last_lines:
            lcd_write(lines)
            last_lines = lines[:]
