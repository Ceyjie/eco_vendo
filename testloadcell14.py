...
# ─── Calibration ─────────────────────────────────────────────────────────────
calibration_factor = -2541.1  # negative to fix inverted load cell
...
OBJECT_CONFIRM  = 3
WARMUP_READS    = 10    # reduced from 20
WARMUP_DELAY    = 0.05  # reduced from 0.1
LOOP_DELAY      = 0.15
HX711_TIMEOUT   = 1.0
MAX_RETRIES     = 5
...
def init_scale():
    ...
    # ── Auto-zero: 5 samples, variance < 500 (faster than before) ────────────
    print("Auto-zeroing", end="", flush=True)
    attempt = 0
    while True:
        samples = [read_raw() for _ in range(5)]  # 5 samples (was 10)
        ...
        if variance < 500:  # relaxed from 200 → faster lock
            tare_offset = avg
            empty_count = 0
            print(f"\nZeroed. Offset={tare_offset:.0f}")
            return True

        attempt += 1
        if attempt >= 20:  # max 20 attempts (was 50)
            tare_offset = avg
            empty_count = 0
            print(f"\nZeroed (forced). Offset={tare_offset:.0f}  Variance={variance:.0f}")
            return True
