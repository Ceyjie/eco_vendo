...
# PIN CONFIG
# ═══════════════════════════════════════════════════════════
PIN_IR_BOTTOM_OFF = 6    # PA6  — gpiod offset (int)
PIN_IR_TOP_OFF    = 1    # PA1  — gpiod offset (int)
...
# SERVO
# ═══════════════════════════════════════════════════════════
# SERVO — simple direct pulsing, no thread/queue complexity
# ═══════════════════════════════════════════════════════════
import mmap, struct
...
def servo_goto(deg, pulses=80):
    """Send PWM pulses directly — no thread, no queue, guaranteed to move."""
    ...
    servo_set_pin(0)  # pin LOW after done
...
# ═══════════════════════════════════════════════════════════
# IR via gpiod with PULL_UP — eliminates ghost triggers
# ═══════════════════════════════════════════════════════════
_ir_request = None

def ir_init():
    """Initialize IR pins via gpiod with internal pull-up."""
    global _ir_request
    # Release from sysfs first if claimed
    ...
    try:
        ...
        print("IR: gpiod with PULL_UP enabled")
    except Exception as e:
        print(f"IR gpiod failed ({e}), falling back to sysfs")
        _ir_request = None
        for pin in [PIN_IR_BOTTOM, PIN_IR_TOP]:
            gpio_setup(pin, "in")
...
def ir_read_bot():
    ...
    return gpio_read(PIN_IR_BOTTOM)
...
def ir_read_top():
    ...
    return gpio_read(PIN_IR_TOP)

def ir_read_both():
    """Read both IR pins in one call."""
    if _ir_request:
        try:
            vals = _ir_request.get_values()
...
def _process_bottle_inner():
    ...
    if pts > 0:
        ...
        # Wait for bottle removal — max 8s
        ...
        # 1.5s after bottle removed → servo returns to 90°
        time.sleep(1.5)
        servo_goto(90)
    ...
    _ir_reset = True  # always reset IR after processing
...
def hardware_loop():
    ...
    while True:
        ...
        # IR reset — re-sync state and add cooldown
        if _ir_reset:
            ir_bot_count = 0
            ir_top_count = 0
            ir_cooldown  = now + 1.0
        ...
        # IR — gpiod with pull-up, 3 consecutive LOW to trigger
...
if __name__ == '__main__':
    ...
    # IR via gpiod with pull-up (no ghost triggers)
    ir_init()
