...
def process_bottle(both_ir, bottom_ir_only):
    """Full bottle processing: weigh → classify → servo → update count"""
    # Show verifying on LCD
    lcd_write(["   VERIFYING...   ", "  Please wait...  ", "", ""])
    time.sleep(0.5)

    # Get weight
    weight = hx711_get_grams() if hx711_ready else None
    ...
    if pts > 0:
        ...
        lcd_write(["   PLEASE WAIT    ", "  Dispensing...   ", f"  {msg.strip()}", ""])
        ...
        # Wait for bottle to be removed (weight drops)
