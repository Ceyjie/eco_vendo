...
def _process_bottle_inner():
    ...
    if pts > 0:
        ...
        # Run servo OUTSIDE lcd_lock — release lock, move servo, re-acquire
        # push to 0°
        t = servo_move(0)
        t.join()          # wait for 0° to complete
        time.sleep(2.0)   # hold 2 seconds at 0°
        t = servo_move(90)
        t.join()          # wait for 90° to complete
