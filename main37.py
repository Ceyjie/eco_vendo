...
# ═══════════════════════════════════════════════════════════
# SERVO (dev/mem high accuracy PWM)
# ═══════════════════════════════════════════════════════════
...
def servo_set_pin(val):
    if not _servo_sysfs and _servo_mem:
        ...
        struct.pack_into("<I", _servo_mem, off,
                         cur | PA20_BIT if val else cur & ~PA20_BIT)
    else:
        gpio_write(PIN_SERVO, 1 if val else 0)
...
def servo_pwm_thread():
    pulse_ms = PULSE_MIN_MS + (90 / 180.0) * (PULSE_MAX_MS - PULSE_MIN_MS)  # 90deg standby
    while servo_active:
        try:
...
# ═══════════════════════════════════════════════════════════
# HX711 — based on working testloadcell code
# ═══════════════════════════════════════════════════════════
HX711_TIMEOUT = 0.5

def hx_begin():
    global hx_fd_dout, hx_fd_sck
    # export with delay like working test
    if not os.path.exists(f"/sys/class/gpio/gpio{PIN_HX711_DOUT}"):
        try:
            with open("/sys/class/gpio/export", "w") as f: f.write(PIN_HX711_DOUT)
        except: pass
    if not os.path.exists(f"/sys/class/gpio/gpio{PIN_HX711_SCK}"):
        try:
            with open("/sys/class/gpio/export", "w") as f: f.write(PIN_HX711_SCK)
        except: pass
    with open(f"/sys/class/gpio/gpio{PIN_HX711_DOUT}/direction", "w") as f: f.write("in")
    with open(f"/sys/class/gpio/gpio{PIN_HX711_SCK}/direction",  "w") as f: f.write("out")
    with open(f"/sys/class/gpio/gpio{PIN_HX711_SCK}/value",      "w") as f: f.write("0")
...
def hx_fast_tare():
    """Fast tare using median of 7 samples — matches working test code."""
    global hx_tare, hx_ready
    # Clear buffer first
    for _ in range(3):
        hx_read_raw()
        time.sleep(0.05)  # small delay between clears
    # Take 7 samples, use median
    samples = []
    for _ in range(7):
        r = hx_read_raw()
    ...
    if len(samples) < 3:
        print("Fast tare failed — no samples")
        return
    samples.sort()
    hx_tare  = samples[len(samples) // 2]
    hx_ready = True
    print(f"Fast tare done. Offset={hx_tare:.0f}")
...
def handle_physical_press(pin):
    ...
    if pin == PIN_BTN_START and s == "IDLE":
        ...
        threading.Thread(target=start_with_cal, daemon=True).start()
...
def display_manager():
    while True:
        # Skip if process_bottle or start_with_cal controls LCD
        if lcd_lock.locked():
            time.sleep(0.1)
            continue
