...
def process_bottle():
    ...
    with bottle_lock:
        ...
        # Re-zero load cell before each detection
        lcd_write([
            "   DETECTED!        ",
            "   Calibrating...   ",
            "   Please wait...   ",
        ...
        if pts > 0:
            ...
            # Servo back home to 90deg standby
            servo_goto(90, hold=0.5)
            servo_goto(90, hold=0)  # ensure standby
            ...
            # Invalid — make sure servo stays at 90 standby
            servo_goto(90, hold=0)
            lcd_write([
                "   INVALID ITEM!    ",
                line1,
...
def handle_physical_press(pin):
    ...
    if pin == PIN_BTN_START and s == "IDLE":
        beep(1)
        # Re-zero load cell before session starts
        def start_with_cal():
            lcd_write([
                "   CALIBRATING...   ",
                "   Please wait...   ",
...
if __name__ == '__main__':
    ...
    # Servo init + standby at 90deg
    servo_init()
    threading.Thread(target=servo_pwm_thread, daemon=True).start()
    time.sleep(0.3)
    servo_goto(90, hold=0.5)  # standby position
